"""
    ...
    Adapted from here: https://github.com/kunaldahiya/pyxclib/blob/master/xclib/evaluation/xc_metrics.py
"""
...
def _setup_metric(X, true_labels, inv_psp=None, k=5, sorted=False):
    ...
    if inv_psp is not None:
        ...
        ps_indices = _get_topk(_psp_wtd.toarray(), num_labels, k)
...
def precision(X, true_labels, k=5, sorted=False):
    ...
    """
    indices, true_labels, _, _ = _setup_metric(X, true_labels, k=k, sorted=sorted)
...
class Metrics(object):
    ...
    def eval(self, pred_labels, K=5, sorted=False):
        ...
        assert compatible_shapes(
            self.true_labels, pred_labels
        ), "Shapes must be compatible for ground truth and predictions"
        indices, true_labels, ps_indices, inv_psp = _setup_metric(
            pred_labels, self.true_labels, self.inv_psp, k=K, sorted=sorted
        )
        _total_pos = np.asarray(true_labels.sum(axis=1), dtype=np.int32)
...
class Metrices(Metrics):
    def __init__(self, true_labels, inv_propensity_scores=None, remove_invalid=False):
        warnings.warn(
            "Metrices() is deprecated; use Metrics().", category=FutureWarning
        )
